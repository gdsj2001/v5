...
// Step/dir + encoder IP (slice executor mode)
...
module axi_stepdir_enc_v2 #(
    ...
    localparam [31:0] REG_VERSION = 32'h0003_0508; // v3.5.8: split GLOBAL_APPLY startup preparation
    ...
    reg exec_prepare_abs_pending;
    reg exec_prepare_minus_pending;
    ...
    reg [31:0] init_steps_abs        [0:N_AXES-1];
    reg [31:0] init_steps_minus1     [0:N_AXES-1];
    reg        init_axis_has_steps   [0:N_AXES-1];
    reg        init_target_dir       [0:N_AXES-1];
    reg        init_any_axis_has_steps;
    ...
    reg        axis_counts_done_q   [0:N_AXES-1];
    reg        axis_quiet_q         [0:N_AXES-1];
    reg        axis_tail_backlog_overrun_q [0:N_AXES-1];
    ...
    reg all_counts_done_q;
    reg all_quiet_q;
    reg tail_backlog_overrun_q;
    reg barrier_ready;
    ...
    always @(posedge s_axi_aclk) begin
        if (!s_axi_aresetn) begin
            ...
            exec_prepare_abs_pending <= 1'b0;
            exec_prepare_minus_pending <= 1'b0;
            ...
            init_any_axis_has_steps <= 1'b0;
            all_counts_done_q <= 1'b1;
            all_quiet_q <= 1'b1;
            tail_backlog_overrun_q <= 1'b0;
            ...
            for (rst_i = 0; rst_i < N_AXES; rst_i = rst_i + 1) begin
                ...
                init_steps_abs[rst_i] <= 32'd0;
                init_steps_minus1[rst_i] <= 32'd0;
                init_axis_has_steps[rst_i] <= 1'b0;
                init_target_dir[rst_i] <= 1'b0;
                ...
                axis_counts_done_q[rst_i] <= 1'b1;
                axis_quiet_q[rst_i] <= 1'b1;
                axis_tail_backlog_overrun_q[rst_i] <= 1'b0;
            ...
            if (exec_flush_pending) begin
                ...
                for (chk_i = 0; chk_i < N_AXES; chk_i = chk_i + 1) begin
                    ...
                    axis_counts_done_q[chk_i] <= 1'b1;
                    axis_quiet_q[chk_i] <= 1'b1;
                    axis_tail_backlog_overrun_q[chk_i] <= 1'b0;
                end
                all_counts_done_q <= 1'b1;
                all_quiet_q <= 1'b1;
                tail_backlog_overrun_q <= 1'b0;
            ...
            if (sync_request) begin
                ...
                if (exec_busy || exec_prepare_abs_pending ||
                    exec_prepare_minus_pending || exec_start_pending) begin
                    exec_fault <= 1'b1;
                    ...
                    exec_busy <= 1'b1;
                    exec_prepare_abs_pending <= 1'b1;
                end
            end else if (exec_prepare_abs_pending) begin
                exec_prepare_abs_pending <= 1'b0;
                exec_prepare_minus_pending <= 1'b1;
                ...
                for (exec_i = 0; exec_i < N_AXES; exec_i = exec_i + 1) begin
                    ...
                    init_steps_abs[exec_i] <= abs_steps_init;
                    init_axis_has_steps[exec_i] <= (delta_masked_init != 0);
                    init_target_dir[exec_i] <= delta_masked_init[31] ^ control_active[exec_i][1];
                end
            end else if (exec_prepare_minus_pending) begin
                any_axis_with_steps = 1'b0;
                exec_prepare_minus_pending <= 1'b0;
                exec_start_pending <= 1'b1;

                for (exec_i = 0; exec_i < N_AXES; exec_i = exec_i + 1) begin
                    if (init_axis_has_steps[exec_i]) begin
                        init_steps_minus1[exec_i] <= init_steps_abs[exec_i] - 1'b1;
                        any_axis_with_steps = 1'b1;
                    end else begin
                        init_steps_minus1[exec_i] <= 32'd0;
                    end
                end
                init_any_axis_has_steps <= any_axis_with_steps;
                ...
                for (exec_i = 0; exec_i < N_AXES; exec_i = exec_i + 1) begin
                    if (init_axis_has_steps[exec_i]) begin
                        target_dir[exec_i] <= init_target_dir[exec_i];
                        axis_dir_changed[exec_i] <= (eff_dir[exec_i] != init_target_dir[exec_i]);
                        if (eff_dir[exec_i] != init_target_dir[exec_i]) begin
                            eff_dir[exec_i] <= init_target_dir[exec_i];
                    ...
                    steps_abs[exec_i] <= init_steps_abs[exec_i];
                    accum[exec_i] <= 32'd0;
                    ...
                    axis_counts_done_q[exec_i] <= 1'b0;
                    axis_quiet_q[exec_i] <= 1'b0;
                    axis_tail_backlog_overrun_q[exec_i] <= 1'b0;

                    axis_active_in_slice[exec_i] <= init_axis_has_steps[exec_i];

                    if (global_cfg_active[CFG_FIRST_STEP_SYNC] && init_axis_has_steps[exec_i]) begin
                        first_step_armed[exec_i] <= 1'b1;
                        steps_remain[exec_i] <= init_steps_minus1[exec_i];
                        dda_num[exec_i] <= init_steps_minus1[exec_i];
                    end else begin
                        first_step_armed[exec_i] <= 1'b0;
                        steps_remain[exec_i] <= init_steps_abs[exec_i];
                        dda_num[exec_i] <= init_steps_abs[exec_i];
                    end
                end
                ...
                if (legal_check_fail) begin
                    ...
                    all_counts_done_q <= 1'b0;
                    all_quiet_q <= 1'b0;
                    tail_backlog_overrun_q <= 1'b0;
                    exec_busy <= 1'b1;
                    start_barrier_wait <= global_cfg_active[CFG_FIRST_STEP_SYNC] && init_any_axis_has_steps;
                end
                ...
                for (chk_i = 0; chk_i < N_AXES; chk_i = chk_i + 1) begin
                    axis_counts_done_q[chk_i] <= !((steps_remain[chk_i] != 0) || first_step_armed[chk_i]);
                    axis_tail_backlog_overrun_q[chk_i] <= (pending_steps[chk_i] > 32'd1);
                    axis_quiet_q[chk_i] <= !(issue_step_pipe[chk_i] ||
                    ...
                    if (!axis_counts_done_q[chk_i])
                        all_counts_done = 1'b0;
                    if (axis_tail_backlog_overrun_q[chk_i])
                        tail_backlog_overrun = 1'b1;
                    if (!axis_quiet_q[chk_i])
                        all_quiet = 1'b0;
                end
                all_counts_done_q <= all_counts_done;
                all_quiet_q <= all_quiet;
                tail_backlog_overrun_q <= tail_backlog_overrun;
                ...
                if (tick_ctr >= slice_ticks_active) begin
                    ...
                    if (!all_counts_done_q || tail_backlog_overrun_q) begin
                        exec_busy <= 1'b0;
                        exec_fault <= 1'b1;
                        exec_overrun <= 1'b1;
