...
原始 `xlnx_atk_lcd.c` 没有“10.1 寸 1024x600”这一完全匹配的硬件型号：其中 `ATK7016` 虽然同为 1024x600，但登记的是 7 寸屏；`ATK1018` 登记的是 10 寸 1280x800。当前产品已按实际组合固定为“7 寸版 LCD 驱动板 + 10.1 寸 1024x600 物理屏 + FT5x26”，显示采用 `timing2`、51.2 MHz；这里复用的是已登记并需板端验证的 1024x600 时序合同，不是把物理屏误认成 `ATK7016`。不得把 `ATK1018`/1280x800 当作当前屏。当前屏规格书仍应用于复核 porch、sync、极性、数据位宽、背光 PWM 和上电时序，最终以生效设备树、时钟 readback、冷启动和物理屏观察证据裁决。
