				       ...
				       unsigned long parent_rate)
{
	...
	rate_div = DIV_ROUND_DOWN_ULL(parent_rate * 1000ll, rate);
	clockout0_div = rate_div / 1000;

	pre = DIV_ROUND_CLOSEST_ULL((parent_rate * 1000ll), rate);
...
static int clk_wzrd_probe(struct platform_device *pdev)
{
	...
	reg_f = (reg & WZRD_CLKFBOUT_FRAC_MASK) >> WZRD_CLKFBOUT_FRAC_SHIFT;
	reg = (reg & WZRD_CLKFBOUT_MULT_MASK) >> WZRD_CLKFBOUT_MULT_SHIFT;
