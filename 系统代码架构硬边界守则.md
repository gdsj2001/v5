...
read_when: [控制链, 状态链, UI到微内核, Command Gate, SHM边界, native adapter, 服务权限, 新服务或IPC, fallback清理, owner下沉]
truth: [控制请求经C UI到framed UDS native gate再到LinuxCNC/HAL/EtherCAT/native；运行actual由微内核/native常驻内存/API拥有，SHM仅作允许字段显示投影]
forbidden: [UI直连LinuxCNC或脚本, Python product action中转控制, 文件/FIFO/JSON控制IPC, SHM/缓存作准入或actual, steady-state fork helper, 第二writer/第二owner, 为测试恢复fallback]
readback: [native owner generation/actual, Command Gate结构化响应, owner指定状态块/API, 动作后的LinuxCNC/HAL/EtherCAT实际状态]
impact: [功能REQ owner, UI命令构造, command_gate, microkernel registry/status API, State Publisher/SHM ABI, HAL/INI, service init/manifest, focused测试与板端验收]
acceptance: [修改前回答0.2问题 -> 对照0.3红线 -> 删除旧路并接入唯一owner -> focused门禁 -> 板端可见改动走VM完整构建/部署/operator path；未上板不得board_verified]
detail_sections: [0.1 一句话总原则, 0.1A 最高准则：可下沉微内核的一律下沉微内核内存, 0.2 AI 修改前必须回答的 7 个问题, 0.3 红线与正确路径, 0.5 当前 owner 落点表, 0.6 触碰式 owner 下沉规则, 3. 控制链硬边界红线, 4. 状态链硬边界红线, 5. 安全提权与系统加固硬边界红线]
<!-- AI_FAST_READ_END -->
...
弃用代码处理口径：发现旧链路、旧分支、旧 helper、旧 JSON/SHM 字段、兼容入口、fallback、shadow path、临时绕路或疑似 AI 自造分支时，默认按“宁可错杀，不放过”处理。若 `功能/需求真源索引.md` 指向的 owner 没有明确要求保留该路径，先物理删除，再用本地编译、必要时由 VM 直接只读挂载 Windows 真源进行 focused/full build 暴露真实缺口；只补回 `功能/` owner 要求的 canonical path，不得为了消除报错恢复旧分支、包一层兼容、改名保留或加开关隐藏。
...
2. **构建即阻断**：凡是违反本守则中任意高压红线的源码状态，必须在 focused compile/contract/static check 或后续完整构建阶段无条件打回；仓库自写通用 preflight/audit 门禁不再作为必跑入口。commit/push 只可作为版本恢复动作，不能替代对应验证门禁或作为完成终点。
...
| 源码真源、VM/板端和 staging | `AGENTS.md` 的 `SOURCE_TRUTH`、`AI_LOCKS_SINGLE_MAINLINE`、`VM_BOARD_SINGLE_OPERATOR` | V5 可维护源码只存在于 Windows `D:\v5` 一棵 Git 工作树：`linux/` 归 Linux kernel/PREEMPT_RT，`linuxcnc/` 归完整 LinuxCNC/V5 native，`board/` 归 HAL/INI/runtime packaging、LVGL、services 和项目工具；Markdown、截图和 Windows/Vivado owner 仍归 `PROJECT_ROOT`，`D:\v3` 只读参考。VM 只能通过 `VM_SOURCE_MOUNT_ROOT` 只读使用同一真源并把 generated files/产物写到 `VM_BUILD_ROOT`；VM 源码副本、板端 `/opt/8ax/...`、`deploy_tmp/`、`bak/`、`repo_ignored/`、旧 build 输出或临时诊断目录不得成为源码或构建输入。发现板端临时修复时，必须回写 Windows canonical owner，重新构建、部署并验证。 |
| VM 清理和 SD 卡重建 | `REQ-MICROKERNEL-SOURCE-REBUILD`、`功能/微内核.md` | VM 旧 build 输出只有在无活动构建且可从 Windows 真源重建时才可直接删除。PetaLinux 工程、`project-spec`、Linux/PREEMPT_RT/LinuxCNC/Yocto 源码或 patch、device tree、XSA/RTL/IP、recipe、集成脚本和其它唯一构建输入不得先删；必须先迁入对应 Windows canonical owner，登记来源与 hash，并以只读真源完成干净系统镜像/SD 卡重建。工具链、下载缓存、board inputs、SD baseline 和恢复制品只有在可重复获取或受控恢复 manifest/hash 已闭合后才可删除。禁止用压缩包、改名目录、另一个 VM 副本或 NAS 可编辑源码树充当迁移完成。 |
