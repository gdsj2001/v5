...
state_v3_check='import math,struct,time,sys,zlib; p="/dev/shm/v3_status_shm"; a=open(p,"rb").read(); time.sleep(0.2); b=open(p,"rb").read(); seq=struct.unpack_from("<I",b,24)[0]; crc=struct.unpack_from("<I",b,28)[0]; mask=struct.unpack_from("<I",b,40)[0]; wid=struct.unpack_from("<I",b,48)[0]; st=struct.unpack_from("<Q",b,56)[0]; sg=struct.unpack_from("<Q",b,64)[0]; scene=struct.unpack_from("<Q",b,72)[0]; unit=struct.unpack_from("<5d",b,160); error=struct.unpack_from("<5d",b,200); digits=struct.unpack_from("<5B",b,240); cg=struct.unpack_from("<Q",b,944)[0]; ct=struct.unpack_from("<Q",b,952)[0]; sf=struct.unpack_from("<I",b,1052)[0]; sb=struct.unpack_from("<Q",b,1024)[0]; dd=struct.unpack_from("<d",b,7128)[0]; actual=zlib.crc32(b[32:],zlib.crc32(b[:24]))&0xffffffff; age=time.monotonic_ns()-st; valid=len(a)==7136 and len(b)==7136 and struct.unpack_from("<5I",b,0)==(0x56355348,4,7136,7136,7104) and seq!=0 and not(seq&1) and (mask&0x303)==0x303 and wid!=0 and st!=0 and sg!=0 and scene!=0 and all(math.isfinite(v) and v>0 for v in unit) and all(math.isfinite(v) for v in error) and digits==(3,3,3,3,3) and cg!=0 and ct!=0 and (sf&1)==1 and (not(sf&32) or (math.isfinite(dd) and dd>=0)) and sb!=0 and actual==crc and 0<=age<=2000000000; print("V5_STATE_ABI_READBACK_OK version=%d size=%d writer_identity=%d seq=%d source_generation=%d scene_generation=%d scene_build=%d cpu_generation=%d age_ns=%d valid_mask=0x%x dd=%s crc=%s"%(struct.unpack_from("<I",b,4)[0],len(b),wid,seq,sg,scene,sb,cg,age,mask,("%.3f"%dd) if sf&32 else "unavailable","ok" if actual==crc else "bad")); sys.exit(0 if valid else 1)'
