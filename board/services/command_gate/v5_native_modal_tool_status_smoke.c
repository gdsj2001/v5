...
int main(void)
{
    ...
    if (!v5_native_modal_tool_status_write_ex(path, 1, "G0 G17 G21 G40 G49 G54 G64 G80 G90 G94 G97", 1, 7, 1, 123.456, 1, 1)) {
        return 3;
    }
    v5_native_readback_init(&readback);
    ...
    unlink(path);
    printf("v5 native modal/tool status: modal=memory tool=T7 length=123.456 invalid=fail_closed\n");
    return 0;
}
