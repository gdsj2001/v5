...
ACTIVE_MODEL_DESCRIPTORS_BY_CANONICAL = {
    "XYZAC_TRT": {
        "registry_id": 1,
        "display": "AC摇篮",
...
def _validated_active_model_descriptor(model: str) -> Dict[str, Any]:
    descriptor = ACTIVE_MODEL_DESCRIPTORS_BY_CANONICAL.get(model)
...
def active_model_descriptor_from_sections(sections: Dict[str, Dict[str, str]]) -> Dict[str, Any]:
    ...
    if kins_axes != traj_axes:
        raise DriveActionError(
            ...
            {
                "model": model,
                "kins_axes": list(kins_axes),
                "traj_axes": list(traj_axes),
            },
        )
...
def active_model_axes_from_runtime_ini() -> Tuple[str, ...]:
    return active_model_descriptor_from_runtime_ini()["active_axes"]
