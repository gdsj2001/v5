...
int main(void)
{
    ...
    v5_remote_metrics_display_text(
        &status,
        cpu0_text,
        sizeof(cpu0_text),
        cpu1_text,
        sizeof(cpu1_text));
    if ((status.valid_mask & V5_STATUS_VALID_CPU_USAGE) == 0U ||
        status.cpu_sample_generation != 7ULL ||
        strcmp(cpu0_text, "cpu0  21%") != 0 ||
        strcmp(cpu1_text, "cpu1  31%") != 0) {
        return 16;
    }
    ...
    if (strcmp(panel.lines[1].mcs_text, "+00029.982") != 0 ||
        strcmp(panel.lines[1].cmd_text, "+00029.982") != 0 ||
        strcmp(panel.lines[1].following_error_text, "+00000.000") != 0) {
        return 13;
    }
    ...
    printf(
        "v5 ui display models: axis=%c mcs=%s err=%s modal=%s points=%u feed=%s cpu=%s/%s\n",
        panel.lines[0].axis,
