    ...
    double point[V5_STATUS_AXIS_COUNT])
{
    ...
    const double b_x = c_x - scene->primary_center[0];
    const double b_z = point[2] - scene->primary_center[2];

    point[0] = scene->primary_center[0] +
        (b_x * scene->primary_cos) + (b_z * scene->primary_sin);
    point[1] = c_y;
    point[2] = scene->primary_center[2] -
        (b_x * scene->primary_sin) + (b_z * scene->primary_cos);
}
...
{
    const double child_x = scene ? scene->child_center[0] - scene->primary_center[0] : 0.0;
    ...
    geometry->child_center[0] = scene->primary_center[0] +
        (child_x * scene->primary_cos) + (child_z * scene->primary_sin);
    geometry->child_center[2] = scene->primary_center[2] -
        (child_x * scene->primary_sin) + (child_z * scene->primary_cos);
    geometry->child_direction[0] = scene->primary_sin;
