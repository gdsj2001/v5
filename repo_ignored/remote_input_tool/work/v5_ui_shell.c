...
static void shell_navigate(void *user_data, V5MainPageActionKind action)
{
    ...
    if (action == V5_MAIN_PAGE_ACTION_NAV_SETTINGS) {
        lv_obj_add_flag(g_main_page.root, LV_OBJ_FLAG_HIDDEN);
        lv_obj_clear_flag(g_settings_page.root, LV_OBJ_FLAG_HIDDEN);
    } else if (action == V5_MAIN_PAGE_ACTION_NAV_MAIN) {
        lv_obj_clear_flag(g_main_page.root, LV_OBJ_FLAG_HIDDEN);
        lv_obj_add_flag(g_settings_page.root, LV_OBJ_FLAG_HIDDEN);
    }
}
