...
- For v5 UI layout-only shells, non-motion page navigation, and non-motion button hit testing, AI should drive the screen itself with simulated screen clicks or the canonical remote-input tool, then record the click target, before/after screenshot, and UI event evidence. Do not wait for the human operator for these routine UI clicks.
