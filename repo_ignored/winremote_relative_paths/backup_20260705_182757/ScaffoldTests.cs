...
{
    string repoRoot = FindRepoRoot(AppContext.BaseDirectory);
    string xaml = File.ReadAllText(Path.Combine(repoRoot, "8ax-win", "src", "8ax.WinRemote", "MainWindow.xaml"));
    string mainWindow = File.ReadAllText(Path.Combine(repoRoot, "8ax-win", "src", "8ax.WinRemote", "MainWindow.xaml.cs"));
    string protocol = File.ReadAllText(Path.Combine(repoRoot, "8ax-win", "src", "8ax.WinRemote", "Protocol", "RemoteMessage.cs"));
    string readme = File.ReadAllText(Path.Combine(repoRoot, "8ax-win", "README.md"));
    string relay = File.ReadAllText(Path.Combine(repoRoot, "lvgl_app", "scripts", "v3_remote_ui_relay.py"));
    string relayStatus = File.ReadAllText(Path.Combine(repoRoot, "lvgl_app", "scripts", "remote_ui_relay_status.py"));
    ...
    Require(true, relay.Contains("\"system_metrics\": self.system_metrics()", StringComparison.Ordinal), "relay info includes system metrics");
    Require(true, relayStatus.Contains("/proc/stat", StringComparison.Ordinal), "relay samples cpu from proc stat");
    Require(true, relayStatus.Contains("/proc/meminfo", StringComparison.Ordinal), "relay samples memory from proc meminfo");
    Require(true, relayStatus.Contains("shutil.disk_usage", StringComparison.Ordinal), "relay samples disk with portable disk usage");
...
static void VerifyWinRemoteBoardTimeSyncContract()
{
    ...
    string vpsDoc = File.ReadAllText(Path.Combine(repoRoot, "功能", "VPS登录与项目对接说明.md"));
    ...
    Require(true, relayStatus.Contains("remote_time_sync_status.json", StringComparison.Ordinal), "relay writes time sync status");
    Require(true, relayStatus.Contains("clock_settime", StringComparison.Ordinal), "relay uses clock_settime instead of shell date");
    Require(true, relayStatus.Contains("_time_sync_completed", StringComparison.Ordinal), "relay tracks one successful time sync");
    Require(true, relayStatus.Contains("if self._time_sync_completed", StringComparison.Ordinal), "relay skips repeated time refresh after success");
    Require(true, relayStatus.Contains("REMOTE_TIME_SYNC_OK", StringComparison.Ordinal), "relay records successful board time overwrite");
    ...
    Require(true, vpsDoc.Contains("网络时间不可用时立即回退 Windows 本机 UTC", StringComparison.Ordinal), "VPS doc documents network-time fallback");
    Require(true, vpsDoc.Contains("每个 relay 进程首次成功覆盖后不重复刷新", StringComparison.Ordinal), "VPS doc documents no repeated refresh");
}
...
static void VerifyUpgradeProgressContract()
{
    ...
    Require(true, mainWindow.Contains("\\u6709\\u66f4\\u65b0", StringComparison.Ordinal), "upgrade button hints available update");
    ...
    Require(true, updater.Contains("主域名", StringComparison.Ordinal), "manifest progress names primary source");
    Require(true, updater.Contains("备用域名", StringComparison.Ordinal), "manifest progress names backup source");
...
static void VerifyOperatorButtonsContract()
{
    ...
    string xaml = File.ReadAllText(Path.Combine(repoRoot, "8ax-win", "src", "8ax.WinRemote", "MainWindow.xaml"));
    string mainWindow = File.ReadAllText(Path.Combine(repoRoot, "8ax-win", "src", "8ax.WinRemote", "MainWindow.xaml.cs"));
    string otaWindow = File.ReadAllText(Path.Combine(repoRoot, "8ax-win", "src", "8ax.WinRemote", "MainWindow.OtaUpgrade.cs"));
    string relayClient = File.ReadAllText(Path.Combine(repoRoot, "8ax-win", "src", "8ax.WinRemote", "Transport", "RemoteRelayClient.cs"));
    string programDialog = File.ReadAllText(Path.Combine(repoRoot, "8ax-win", "src", "8ax.WinRemote", "ProgramDirectoryDialog.cs"));
    string logDialog = File.ReadAllText(Path.Combine(repoRoot, "8ax-win", "src", "8ax.WinRemote", "Diagnostics", "ClientLogPreviewDialog.cs"));
    string readme = File.ReadAllText(Path.Combine(repoRoot, "8ax-win", "README.md"));
    string buttonDoc = File.ReadAllText(Path.Combine(repoRoot, "8ax-win", "界面按钮.md"));
    string mockRelay = File.ReadAllText(Path.Combine(repoRoot, "8ax-win", "tools", "mock-relay", "Program.cs"));
    string boardRelay = File.ReadAllText(Path.Combine(repoRoot, "lvgl_app", "scripts", "v3_remote_ui_relay.py"));
    ...
    Require(true, boardRelay.Contains("PROGRAM_UPLOAD_DIR", StringComparison.Ordinal), "board relay owns program upload directory");
    Require(true, boardRelay.Contains("save_uploaded_program", StringComparison.Ordinal), "board relay saves uploaded program atomically");
    Require(true, boardRelay.Contains("list_program_files", StringComparison.Ordinal), "board relay lists program directory");
    Require(true, boardRelay.Contains("read_program_file", StringComparison.Ordinal), "board relay reads program file content");
    Require(true, boardRelay.Contains("delete_program_file", StringComparison.Ordinal), "board relay deletes program file");
    Require(true, boardRelay.Contains("ota_upgrade_not_implemented", StringComparison.Ordinal), "board relay rejects OTA until OTA client exists");
    Require(true, readme.Contains("bottom `读取日志` button", StringComparison.Ordinal), "README documents read-log button");
    ...
    Require(true, buttonDoc.Contains("自动换行", StringComparison.Ordinal), "button doc documents diagnostics wrapping");
    Require(true, buttonDoc.Contains("[redacted]", StringComparison.Ordinal), "button doc documents diagnostics redaction placeholder");
    ...
    Require(true, buttonDoc.Contains("打开系统G代码", StringComparison.Ordinal), "button doc documents open system gcode button");
    Require(true, buttonDoc.Contains("OTA升级", StringComparison.Ordinal), "button doc documents OTA upgrade button");
}
...
static void VerifyDiagnosticsDisplaySanitization()
{
    ...
    Require(true, summary.Contains("诊断摘要", StringComparison.Ordinal), "diagnostics summary is default operator view");
    Require(true, summary.Contains("关键状态", StringComparison.Ordinal), "diagnostics summary includes status section");
...
static void VerifySingleExePublishContract()
{
    ...
    string winReadme = File.ReadAllText(Path.Combine(repoRoot, "8ax-win", "README.md"));
    string uploadRule = File.ReadAllText(Path.Combine(repoRoot, "8ax-win", "VPS自动上传规则.md"));
    ...
    Require(true, uploadRule.Contains("正式发布不得停在本地 `zip`、`manifest.json`", StringComparison.Ordinal), "VPS doc requires direct upload after generation");
...
static string FindRepoRoot(string startDirectory)
{
    ...
    string? fallback = Environment.GetEnvironmentVariable("RE_V3_ROOT");
    if (!string.IsNullOrWhiteSpace(fallback) &&
        File.Exists(Path.Combine(fallback, "8ax-win", "tools", "mock-relay", "8ax.MockRelay.csproj")))
    {
