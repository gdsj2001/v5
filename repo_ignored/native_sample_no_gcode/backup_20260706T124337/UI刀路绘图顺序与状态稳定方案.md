...
当前主页面可见坐标必须同时显示当前模态、机械坐标系、加工坐标系和 A/C。加工坐标系以当前 active WCS 为显示基准原点，坐标面板中的加工坐标列显示 `0,0,0` 基准（A/C 同列按同一 active WCS 基准显示零位），其它机械坐标、A/C 轴线、刀尖/刀柄姿态和程序投影只能根据 native/WCS/G53/A-C geometry/参数 owner 的相对位置建立；不得把 `cmd_mcs`、驱动表、设置页表格、profile、旧缓存或默认 0 当作加工坐标系真源。`cmd_mcs` 继续只用于红色 commanded 点、理论轨迹和跟随误差诊断。
