...
- VPS 端签名私钥/公钥对应位置是 `/opt/z20-auth/secrets/device-auth-signing-private.pem` 和 `/opt/z20-auth/public/device-auth-signing-public.pem`；板端验签公钥位置是 `/etc/6x-cnc/device_auth_public.pem`。
