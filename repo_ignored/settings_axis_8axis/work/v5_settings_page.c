...
static void log_button_event(V5MainPageActionKind action, const V5MainPageActionReport *report)
{
    ...
    fprintf(fp, ",\"ok\":true,\"implemented\":false,\"layout_only\":true");
    if (report) {
        fprintf(fp, ",\"prepared\":false,\"local_only\":true,\"command_kind\":%d,\"command_name\":", (int)report->command.kind);
