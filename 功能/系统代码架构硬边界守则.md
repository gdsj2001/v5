...
read_when: [控制链, 状态链, UI到微内核, Command Gate, SHM边界, native adapter, 服务权限, 新服务或IPC, fallback清理, owner下沉, CPU0/CPU1划分, affinity或cpuset, IRQ隔离, UI卡死不影响运动]
truth: [控制请求经C UI到framed UDS native gate再到LinuxCNC/HAL/EtherCAT/native；运行actual由微内核/native常驻内存/API拥有，SHM仅作允许字段显示投影, CPU0是只容纳持续运动截止期必需链路的专属实时域；Start后运动不得依赖CPU1继续推进]
forbidden: [UI直连LinuxCNC或脚本, Python product action中转控制, 文件/FIFO/JSON控制IPC, SHM/缓存作准入或actual, steady-state fork helper, 第二writer/第二owner, 为测试恢复fallback, UI/Publisher/镜像/日志/诊断或非运动IRQ进入CPU0, CPU1过载时把非运动工作迁入CPU0]
readback: [native owner generation/actual, Command Gate结构化响应, owner指定状态块/API, 动作后的LinuxCNC/HAL/EtherCAT实际状态, task affinity/cpuset/scheduler, IRQ affinity, servo latency/overrun与EtherCAT DC/WKC]
impact: [功能REQ owner, UI命令构造, command_gate, microkernel registry/status API, State Publisher/SHM ABI, HAL/INI, service init/manifest, CPU affinity/cpuset/IRQ/workqueue配置, focused测试与板端验收]
acceptance: [回答0.2 -> 对照0.3 -> 删除旧路并接入唯一owner -> focused门禁 -> 受影响构建/必要current产物 -> 部署/operator path；未上板不得board_verified, active-model连续运动中冻结UI并压满CPU1仍无运动停顿、servo overrun或DC/WKC异常]
