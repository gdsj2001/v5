...
- 刀路 UI 可以用 SHM 消费 30ms UI 显示投影：编码器反馈机械坐标、运动学理论坐标、`runtime_modal_text`、`spindle_speed_rpm`、`linear_velocity_mm_per_min`、`feedrate_override`、`spindle_override`；WCS、TLO/tool、A/C geometry、G53、RTCP 动作证明、安全、诊断和其它 actual 仍直连微内核/native 内存，不得用 request、dirty、设置文件、helper cache、旧 SHM 字段或旧快照拼 active。
...
刀路显示必须使用同一 generation 的状态。SHM 只提供 30ms UI 显示投影：编码器反馈机械坐标、运动学理论坐标、`runtime_modal_text`、`spindle_speed_rpm`、`linear_velocity_mm_per_min`、`feedrate_override`、`spindle_override`；该投影必须由常驻 State Publisher 或登记 native publisher 从微内核/native 内存采样维护，不得由周期短脚本、硬盘 JSON/result 或一次性 helper 搬运状态。WCS、TLO/tool、A/C geometry、G53、RTCP 动作证明、安全、诊断和其它 actual 必须由 UI 状态 provider 直连微内核/native 内存或登记 API 读取。每一帧先把 SHM 显示投影和直连微内核状态按 generation 收敛为 scene/model，再刷新各图层；不得一个图层读 SHM、另一个图层读缓存或 JSON，也不得在动态帧里重新读取 G-code 文件、资源文件或运行结果文件。
...
| `runtime_modal_text` | G-code/M-code 控制微内核/LinuxCNC runtime modal 后，经 30ms SHM UI 显示投影发布 | 模态文字和 RTCP 图层显示 | 缺失则 RTCP unknown/degraded；request/dirty 只能显示 pending，不按按钮请求提前切 active |
...
| `rtcp_actual_enabled` | 只能从 `runtime_modal_text` 或直连微内核/native modal actual 派生；不是独立真源 | RTCP 图层布尔显示投影 | modal 缺失时 invalid；不得由 request/dirty/设置文件、非同源 helper/cache 或旧快照派生 |
| `tool_offset` / `display_tool_length_mm` | LinuxCNC/tool native owner 直连微内核/native 内存或原生 API | 刀长、红点 Z 计算、刀柄线 | 缺失则隐藏相关动态图层 |
...
RTCP/G53/A/C 的动作证明和几何真源统一命名为 same-generation native actual，不同步到 SHM。RTCP 的控制因果是：G-code/M-code 进入微内核/LinuxCNC，微内核 runtime modal 成为 actual owner；State Publisher 只可把该 owner 的 `runtime_modal_text` 投影到 SHM 供 UI 显示，UI/provider 也可直连回读 modal owner 并与 SHM generation 收敛。WCS、TLO、A/C geometry 和 G53 仍必须来自微内核/native 内存或登记 API，不允许 UI 从旧 SHM 字段、多个缓存或 JSON 自行拼帧。`rtcp_actual_enabled` 如保留，只能是从该 modal actual 或 `runtime_modal_text` 派生的显示字段。RTCP request 只能在界面显示 pending；只有后续 native modal actual 确认 active 后，RTCP 图层才能切 active。
...
总规则：开机读内存建 WCS/MCS/A/C 轴线并 fit；打开 G-code 建黄线并 fit；RTCP/模态文字可读 SHM 显示投影或直连 modal owner；之后只机械坐标值、模态文字和速度/倍率显示投影驱动动态层，不得重建。
...
- RTCP 显示状态可从 G-code/M-code 控制后的 SHM `runtime_modal_text` 或直连微内核 runtime modal actual 派生；产品动作证明必须直连 native actual。运行 G-code 时可以复用上一帧有效 RTCP modal actual 缓存；RTCP 开关、模态变化、异常、无有效缓存或缓存过期时必须重新读，不能用缓存假装正常。
...
状态层必须以同一帧 SHM UI 显示投影/`ReV3StatusView` 的有效机械坐标作为绘图闸门，并以直连微内核/native 的 WCS、A/C geometry、RTCP 动作证明和 tool 状态作为结构参数。只要 SHM 坐标和必要 native 状态已经提供有效数据，状态层就必须显示；不得因为尚未打开 G-code、黄色轨迹未解析、程序缓存为空或 Program Open 失败而隐藏 MCS/WCS/A/C/坐标读数。`status_epoch == 0`、SHM stale、`mcs[0..2]` 非 finite 或仍为不可用哨兵值时，状态层不得绘制种子图、假坐标系、默认 0 位姿或占位点，只能隐藏动态状态层或显示 degraded。
...
`empty_status_scene`、假两点背景、初始种子坐标、默认 A/C=0 或默认 WCS/MCS 原点都属于退役路径；缺少有效 MCS 时只能隐藏状态依赖图层或显示 degraded 文本，不能用伪造几何让界面看起来“已就绪”。
## 3. 绘图语义
...
绿色点表示同一帧 `mcs[0..2]` 的 actual 机械反馈点或刀柄上点。刀柄线使用同一帧刀长/姿态状态，只做显示，不反推 RTCP 真值。
...
- A/C 中心点用 2px 中心标记，只表示几何中心，不替代轴线或刀尖点；A/C 轴中心 marker 必须按同一份 scene 几何，不得用屏幕红线、标签位置或 UI 像素坐标反推。
...
- 开机默认视角必须是 3D，`view_3d` 进入 3D；3D 视角投影 owner 是 `lvgl_app/src/v3_ui_toolpath_3d_view.c`，相机方向为 X+ Y- Z+。
...
- RTCP 图层只按 G-code/M-code 控制后的微内核 runtime modal actual 显示；UI 侧可读取 SHM `runtime_modal_text` 显示投影，也可通过状态 provider 直连读取 native modal/RTCP actual。`rtcp_actual_enabled` 或等价布尔字段只能是该 modal actual 或 `runtime_modal_text` 的派生显示投影。request、dirty、按钮状态、设置文件和非同源 helper/cache 只能显示 pending 或诊断，不得提前切 active；State Publisher 内部上一帧有效 RTCP 缓存只允许作为运行期回读优化，开关/异常/无有效缓存时必须重读。
...
- C 蓝线中点必须是 `(G53_C_X, G53_C_Y, G53_A_Z)`。显示层 A/C 投影变换必须使用 `ac_center_x = G53_C_X`、`ac_center_z = G53_A_Z`、`ac_offset_z = 0`。
- MCS ready 后的首次有效绘图中，A 红线中心必须是 `(当前加工坐标系 X, G53_A_Y, G53_A_Z)`。C 蓝线的空间位置由直连微内核/native 的 A/C 几何生成，初始方向与 MCS Z 平行，后续只能按同一条 UI A/C 显示投影中的 A 值围绕 A 轴红线做空间旋转。开机首帧没有有效机械坐标时不得提前绘制 A/C 轴、MCS/WCS 坐标系或红点。
...
- 清空状态层只允许发生在明确的对象销毁，或长期 SHM 显示投影不可用并进入明确 fail-closed 空场景的路径；页面重新创建后也必须等待首次有效 SHM UI 显示投影和必要 native actual，按开机规则重新建立 WCS/MCS、A/C 轴线和 fit。更换程序只允许清理黄色轨迹，不得清空或重建状态层。清空状态层不得出现在 30ms 普通刷新路径里。
