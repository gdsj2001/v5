...
- 静态/结构层不得固定 30Hz 重建。WCS/MCS 原点和轴线、A/C 轴线和中心点、标签锚点等状态层对象在主页面创建/开机首帧先按 v3 视觉骨架显示；首次读到有效 SHM UI 显示投影和必要 native actual 后，只用真实状态覆盖这套对象的投影，不另建第二套对象。静态 G-code 黄线只在 Program Open/Load 成功时解析、缓存或重建；screen fit 只在首次有效状态层建立和黄色程序线出现时自动重建，其中黄色程序线出现后的 fit 必须把状态层和程序层全部当前可见元素一起纳入。视角、手势和窗口变化只能更新显示 transform 或裁剪后的投影输出，不得触发 fit、状态层结构或 G-code 重建；XY/XZ/YZ/3D 视角切换必须清除手势旋转并恢复对应默认视角。RTCP actual 开启后，A/C 显示机械值变化只允许按已缓存黄色程序点驱动投影刷新；RTCP OFF 时 A/C 变化不得触发黄色轨迹重投影。
...
- 静态 G-code 黄线必须在 Program Open/Load 后台任务完成并写入当前程序元数据后，由主页面后台缓存生成；程序页双击前台成功路径可以只做本地文件名/路径存在性校验、启动 Program Open/Load 事务并切回主页面显示 loading，但“程序已打开/可 Start”的成功态必须以 resident runtime identity/handle 和只读程序模型/运行内存副本已登记为最低条件。不得使用 `program_open_cc` 中介；不得把路径存在、UI 元数据写入、黄线解析任务启动或 loading 状态冒充可运行成功。前台返回主页面不需要同步等待 LinuxCNC `program_open`、黄色预览解析、投影缓存、fit、MDI 全量读取或其它大 G-code 预览缓存完成，但 Start/Resume 热路径不得再读程序文件、生成运行副本或补建 resident handle。黄色程序点必须保留打开程序自身解析出的 G54-G59.3 模态；显示时按该程序点所属 WCS 的 native/interpreter offset 把程序 XYZ 放入机械坐标显示层，再与 MCS/WCS/A/C/刀柄对象一起投影，不得把运行时 active WCS 二次套到已带 WCS 模态的程序点上。没有对应 WCS offset 的点不得用默认 G54、UI 缓存或当前 active WCS 乱画。在大型 G-code 文件解析和刀路加载期间，UI 应当显示一个加载指示器（如 LVGL Spinner 或 Loading 进度动画），解析及黄色轨迹载入后的全部可见元素统一 fit 缓存完成后自动将其隐藏，以提供对大型文件加载进度的良好交互反馈。30Hz SHM 显示路径不得重新解析 G-code、不得重新读取程序文件、不得全量重建黄色路径模型。屏幕 fit 只在首次有效状态层建立和黄色程序线出现时自动重建；黄色程序线出现后的 fit 必须把黄线、WCS/MCS、A/C、红点、绿点、刀柄线和标签锚点作为同一个候选集合重新计算。后续同一帧 SHM 显示投影 + native 状态只驱动已有 WCS/MCS、A/C 轴线和黄色程序线投影。RTCP actual 开启且 A/C 显示机械值变化时，只允许按已缓存程序点刷新投影和必要的 `lv_line_set_points()`；RTCP OFF、无 A/C 投影变化或缓存命中时不得全量重投影黄线、不得全量重写黄色 `lv_line_set_points()`。
