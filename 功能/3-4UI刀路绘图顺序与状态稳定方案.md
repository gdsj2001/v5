...
引用需求真源：`REQ-DOC-SINGLE-SOURCE`、`REQ-NATIVE-OWNER-FIRST`、`REQ-LINUXCNC-COMMAND-GATE`、`REQ-PARAM-MEMORY-LIGHTWEIGHT-SAVE`、`REQ-SHM-DISPLAY-PROJECTION`、`REQ-G92-NATIVE-RUNTIME`、`REQ-WCS-NATIVE-OWNER`、`REQ-ROTARY-UNWRAP-ABSOLUTE-PROOF`、`REQ-ROTARY-REBASE-NATIVE-GATE`、`REQ-RTCP-G53-NATIVE-ACTUAL`、`REQ-RTCP-NATIVE-STATUS-SOURCE`、`REQ-GCODE-RUN-HOT-PATH`。
...
- G-code 程序模型、轨迹缓存、LOD/segment 缓存和页面图层资源属于 0-1 的受控资源域：大块内存优先用独立 pool/arena 或 `mmap` 生命周期管理；只有 program close、程序切换、页面销毁或受控 reload 的空闲边界可释放并 trim，30ms 动态绘图、Start/Resume、急停和伺服相关路径不得分配/trim。
...
总规则：开机先建立非真值 UI 结构骨架；首次有效 SHM 显示投影和必要 native actual 到达后覆盖 WCS/MCS/A/C 轴线并 fit；打开 G-code 建黄线并 fit；RTCP/模态文字、当前刀号和对应刀长必须直连 native modal/tool owner；之后只机械坐标值、native 模态/tool 状态和速度/倍率显示投影驱动动态层，不得重建。

- 开机/页面创建顺序固定为：先立即建立非真值 UI 结构骨架并接管 framebuffer，不等待 SHM/native readiness；首次读到有效 30ms SHM UI 显示投影，同时直连读到必要微内核/native WCS、A/C geometry 和动作证明状态后，用真实状态覆盖 WCS/MCS、A/C 轴线状态层结构，再生成第一次 fit。
...
- fit 只有开机/页面创建后首次读到有效 SHM UI 显示投影并直连读到必要 native actual 覆盖状态层时自动重建一次，以及打开 G-code 成功并显示黄色程序线时自动重建一次。
...
刀路预览可以为了板端内存和绘制开销对大程序做显示层抽样，但必须满足以下边界：
  ...
  - Program Open 的黄色程序层必须使用独立程序预览容量，参考 v3 `RE_V3_TOOLPATH_MAX_POINTS = 512` 或当前 v5 等价预览容量；不得把 `/dev/shm/v3_status_shm` 的 30ms 动态轨迹点数上限 `V5_STATUS_TRAJECTORY_POINT_COUNT = 16` 当作 G-code 程序预览容量、程序完整性判断或黄色程序 fit 来源。SHM 16 点只属于动态显示投影 ABI，只能用于动态 overlay，不得导致 `cc.ngc` 这类 5 圈弹簧程序被截成 5 点或 16 点方形/折线。
...
- A/B/C 中心点用 2px 中心标记，只表示机床几何中心，不替代轴线或刀尖点；A/B/C 轴中心 marker 必须按同一份 G53/native scene 几何，不得用屏幕红线、标签位置、当前 WCS offset、加工坐标系偏移或 UI 像素坐标反推。设置页 G53 表的显示顺序必须为 A 中心、B 中心、C 中心、对刀仪、5 方向检测仪；A/B/C 中心的 X/Y/Z 均必须来自对应 `G53_A_*`、`G53_B_*`、`G53_C_*` 或等价 native geometry readback。WCS offset 只移动加工坐标系和程序点，不得移动 A/B/C 机床旋转轴中心。
...
- 手势显示态当前 owner 是 `D:\v5\board\app\src\v5_main_page.c` 的输入/视图 transform 路径；若手势状态机继续变复杂，必须下沉到新的 `v5_toolpath_*` 或 `v5_main_page_*` 子 owner。`manual_scale`、`manual_screen_rotate_deg`、`manual_wcs_x_rotate_deg`、`manual_wcs_z_rotate_deg` 只影响显示 transform。按 XY/XZ/YZ/3D 视角按钮只回到按钮对应的默认视角投影，不得重建 fit、WCS/MCS、A/C 轴线或黄色程序模型。
...
- C 蓝线中点必须是 `(G53_C_X, G53_C_Y, G53_C_Z)` 或等价 native geometry readback；显示层 A/C 投影变换必须使用 G53/native C 中心，不得使用当前模态加工坐标系 Z 偏移、`G53_A_Z`、UI 默认值、自建参数表值或旧缓存替代。
- MCS ready 后的首次有效绘图中，A 红线中心必须是 `(G53_A_X, G53_A_Y, G53_A_Z)` 或等价 native geometry readback；不得使用当前模态加工坐标系 X 偏移、自建参数表或 UI 默认值替代。C 蓝线的空间位置由直连微内核/native 的 A/C 几何生成，初始方向与 MCS Z 平行，后续只能按同一条 UI A/C 显示投影中的 A 值围绕 A 轴红线做空间旋转。开机首帧允许绘制非真值视觉骨架；没有有效机械坐标和 native geometry 时，不得绘制真实 A/C 姿态、红/绿 actual 点、真实刀柄线或声明 A/C 几何 actual。
...
- State Publisher nice/affinity 只能作为板端 A/B 验证项。默认不得提升为实时线程，不得主动绑定或抢占 CPU0；只有板端证据证明 LinuxCNC/HAL/EtherCAT 实时线程无新增 overrun、Broker 急停/停止延迟不恶化、每核 CPU 分布合理时，才允许调整普通调度参数。nice/affinity 不能作为掩盖 UI 30ms 全量重绘的方案。
...
| SHM ABI 白名单和 30ms 显示投影结构 | `D:\v5\board\app\include\v5_status_shm.h` |
