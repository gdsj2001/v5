...
- 当前源码 owner 以 VM `/root/Desktop/v5` 的 v5 主线为准。本文出现的 v3 名称、常量和视觉对象只作为操作员视觉参考，不是源码 owner，不允许恢复 v3 后端、旧缓存、兼容壳或平行实现；实际文件变更前必须先按本文第 4 节更新 owner。
...
状态层必须以同一帧 SHM UI 显示投影/`ReV3StatusView` 的有效机械坐标作为实际坐标绘图闸门，并以直连微内核/native 的 WCS、A/C geometry、RTCP 动作证明和 tool 状态作为结构参数。只要 native 运行内存/API 能读到 actual 坐标，即使尚未打开 G-code、程序为空或 machine off，State Publisher 也必须把 MCS/A-C 坐标、转速、进给速度和主轴/进给倍率投影到 `/dev/shm`，并把当前模态、当前刀号和刀长发布到 native modal/tool 状态块或等价 RAM API；UI 必须用真实状态覆盖开机骨架并显示状态层；不得因为尚未打开 G-code、黄色轨迹未解析、程序缓存为空或 Program Open 失败而隐藏 MCS/当前 WCS/A/C/坐标读数。该路径禁止读取硬盘文件、设置表、驱动表、profile 或旧缓存补坐标。`status_epoch == 0`、SHM stale、`mcs[0..2]` 非 finite 或仍为不可用哨兵值时，UI 仍必须保留包含当前 WCS 与 A/C 对象的开机结构骨架、红/绿屏幕锚点和 degraded/UNAVAILABLE 文字，但不得绘制种子图、假坐标数值、默认 0 位姿、真实 actual 刀柄线或真实 A/C 姿态。
...
- 开机默认视角必须是 3D，`view_3d` 进入 3D；当前 v5 3D 视角投影 owner 是 `/root/Desktop/v5/app/src/v5_toolpath_display.c`，v3 向量只作为视觉参考。相机方向为 X+ Y- Z+ 的低角度正交视角，正交投影基向量必须与 v3 一致：screen right = `(0.7071067811865476, 0.7071067811865476, 0.0)`，screen up = `(-0.4082482904638631, 0.4082482904638631, 0.8164965809277261)`。在默认 3D 屏幕上 X 轴应向右下，Y 轴应向右上，Z 轴应竖直向上；v5 不得用近似经验公式、按钮选中态、手工屏幕坐标或其它 3D 斜投影替代这组向量，也不得让开机骨架使用另一套手写斜率。
...
- 手势显示态当前 owner 是 `/root/Desktop/v5/app/src/v5_main_page.c` 的输入/视图 transform 路径；若手势状态机继续变复杂，必须下沉到新的 `v5_toolpath_*` 或 `v5_main_page_*` 子 owner。`manual_scale`、`manual_screen_rotate_deg`、`manual_wcs_x_rotate_deg`、`manual_wcs_z_rotate_deg` 只影响显示 transform。按 XY/XZ/YZ/3D 视角按钮只回到按钮对应的默认视角投影，不得重建 fit、WCS/MCS、A/C 轴线或黄色程序模型。
...
| 职责 | 当前 v5 owner |
| --- | --- |
| LVGL 主页面对象、图层顺序、事件入口、刷新 glue | `/root/Desktop/v5/app/src/v5_main_page.c`、`/root/Desktop/v5/app/src/v5_main_page.h` |
| 主页面按钮/动作薄转发 | `/root/Desktop/v5/app/src/v5_main_page_actions.c`、`/root/Desktop/v5/app/src/v5_main_page_actions.h` |
| SHM ABI 白名单和 30ms 显示投影结构 | `/root/Desktop/v5/app/include/v5_status_shm.h` |
| UI SHM reader、stale/degraded/unavailable 显示模型 | `/root/Desktop/v5/app/src/v5_status_shm_reader.c`、`/root/Desktop/v5/app/src/v5_ui_status_view.c`、`/root/Desktop/v5/app/src/v5_ui_status_view.h` |
| 屏幕 projector、fit-lock、点投影、3D 基向量 | `/root/Desktop/v5/app/src/v5_toolpath_display.c`、`/root/Desktop/v5/app/src/v5_toolpath_display.h` |
| G-code 程序运行内存副本、程序预览点、打开/关闭生命周期 | `/root/Desktop/v5/services/program_runtime/v5_program_runtime.c`、`/root/Desktop/v5/services/program_runtime/v5_program_runtime.h` |
| State Publisher SHM mmap/write helper | `/root/Desktop/v5/services/state_publisher/v5_status_shm_mmap.c`、`/root/Desktop/v5/services/state_publisher/v5_status_shm_writer.c` |
| 当前未拆出的 scene/projection/screen-fit/pose/overlay/diagnostic/gesture 子 owner | 先在上述 v5 owner 内做最小修复；新增复杂逻辑必须下沉为 `/root/Desktop/v5/app/src/v5_toolpath_*` 或 `/root/Desktop/v5/app/src/v5_main_page_*` 子 owner，不得堆回超大主页面文件 |
...
源码修改后至少运行当前 v5 VM 源码门禁；如果下列目标不存在，必须先补齐对应 v5 smoke/contract 测试，不能回退到旧 `lvgl_app` 测试路径：
