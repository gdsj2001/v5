...
read_when: [VPS 登录, 授权, private/public 下载, drive profile, 逐轴速度前馈维护, OTA package, remote relay, WinRemote更新, 厂家远程 SSH]
truth: [授权身份 -> package/profile 或当前设备反向 SSH 登记 -> 板端校验/加载或 VPS tunnel/host-key readback -> consumer, remote relay使用设备独立TLS证书与客户端challenge-response/scoped RAM session, WinRemote更新先验ECDSA P-256分离签名再解析manifest且安装子进程二次验签验hash, WinRemote正式EXE一旦更新必须在同一次发布上传VPS并完成主备域签名三件套回读, 驱动整定 SDO 不进入参数映射且普通链路不写, 显式逐轴速度前馈维护只按型号profile白名单在Machine Off窗口读写H08.19并通过H0E.01临时保存策略与software reset后readback闭合, 显式复位驱动可按型号 profile 恢复厂家默认, PDO 固定布局不进入用户参数表]
forbidden: [public fallback 冒充授权, 客户分叉产品代码, 未验签manifest或只验hash的下载, 复用设备授权/SSH/relay私钥作更新签名, 正式publish目录用-SkipUpload生成新EXE或本地更新后不上传VPS, relay明文/匿名/URL token/证书忽略fallback, PowerShell ExecutionPolicy Bypass更新器, 下载验证后安装前不复核的TOCTOU, 把访问记录 IP 当 SSH 地址, Cloudflare 代理 SSH, 自动接受 host key, SSH/FIFO 产品控制旁路, 启动或设置驱动自动写 H01/H05/H08 整定 SDO, 任意index/subindex写入或统一前馈值覆盖五轴, 单次H08.19变化超过10.0%, EEPROM policy未恢复0或未在software reset后再读回就宣称成功, 用驱动参数表生成或改写 HAL/XML/PDO, 把复位驱动或前馈维护例外扩成普通 SDO 写入口]
readback: [license/entitlement, package/profile hash/version, 板端加载 identity, 逐轴前馈profile/map identity与H05.19/H08.18/H08.19/H0E.01/reset后持久值/全从站OP/error_code, relay certificate fingerprint/device_id/client_id/scopes/session, WinRemote manifest key_id/signature/release_sequence/package size/hash/installer reverify, device tunnel status, SSH host-key/device readback]
impact: [WinRemote与更新发布脚本, remote relay, dealer/factory client, IP访问记录弹窗, drive mapping, 逐轴速度前馈维护动作, OTA, settings actiond]
acceptance: [身份与产物 hash 闭合；relay明文/匿名/错误MAC/重放/过期/越权/错误证书均fail-closed且正确WinRemote证书固定与scope可走原始路径；WinRemote在解析manifest字段前验证分离签名，包下载后与安装前都验证同一签名manifest绑定的size/hash，篡改manifest/signature/package或回退release_sequence均拒绝；每次正式EXE更新同次上传VPS且VPS原始文件与主备HTTPS的zip/manifest/signature完全一致并验签通过；远程 SSH 必须证明当前设备弹窗 -> 同一设备 ID 的 VPS loopback 隧道 -> 板端 SSH 主机身份与命令回读；普通链路无整定 SDO 写路径且 XML 不含 H01/H05/H08 初始化写入；逐轴前馈维护每次只处理一个已绑定从站、H05.19必须为1、H08.18只读、H08.19单步不超过10.0%，成功和失败回滚都必须恢复H0E.01=0、software reset后再读回并保持Machine Off；复位驱动仅在 Machine Off/无运动窗口执行厂家恢复默认并保持 Machine Off]
...
| `/remote/program/upload` | 上传 Windows 本地 G-code 到板端“打开程序”唯一目录 `/opt/8ax/v5/gcode/golden` | 不打开程序、不运行程序、不替代 UI/operator 路径 |
| `/remote/program/list` | 列出 `/opt/8ax/v5/gcode/golden` 直属目录中的受支持 G-code 文件及大小、时间、SHA256 | 不递归目录、不扫描 `/opt/8ax/v5/gcode` 根目录或其它目录冒充板端程序列表 |
| `/remote/program/file` (`GET`) | 查询单个 basename 文件；`content=1` 时读取可编辑 UTF-8 文本 | 不接受路径穿越、符号链接或超过编辑上限的内容 |
| `/remote/program/file` (`DELETE`) | 删除用户明确选择的单个 basename 文件并返回删除前证据 | 不删除目录、不删除未选择文件、不影响当前已加载程序真值 |
...
- 原子写入。
- 返回 SHA256 和字节数读回证据。
- 文件扩展名只允许 `.ngc`、`.nc`、`.tap`、`.gcode`，单文件大小上限与板端 program runtime 一致为 2 MiB；空文件拒绝。
- 上传必须先写同目录临时文件、`fsync` 后原子替换；未明确 `overwrite=1` 时同名文件返回冲突，不得静默覆盖。
- `list/file/upload/delete` 只管理文件，不得触发 Program Open、Start、LinuxCNC/HAL 命令或任何运动。
- 这些文件接口不依赖 UI framebuffer ready；只要 relay 和程序目录可用即可读取、上传或整理，错误必须返回稳定 JSON `error` 与中文客户端可显示的原因。
- WinRemote 必须优先适配板端当前 `v5.remote_program_*.v1` 返回，同时保留对既有 `re.v3.remote_program_*.v1` 字段兼容；HTTP 非成功响应必须显示板端 `error/message`，不得只显示泛化状态码。
- relay 程序目录必须与板端 `v5_ui_shell_program_scan.c` 的 `PROJECT_ROOT/gcode/golden` 使用同一 `PROJECT_ROOT` 派生值；WinRemote 不得自行猜测或拼接另一个目录。上传、列表、打开修改、保存和删除必须全部作用于这个同一目录，确保远程窗口与板端“打开程序”页面看到同一组文件。
...
`REQ-REMOTE-SSH-MAINTENANCE` 只提供厂家维护 shell，不是产品运动控制、设置保存、部署真源或 operator path。厂家控制台标题栏不得放置 `远程连接`；按钮只位于当前设备的 `IP访问记录` 弹窗底部右侧，并明确显示当前 6 位设备 ID 和 `VPS` 路由。访问记录 IP 只用于说明该弹窗属于哪台设备以及最近出站网络，不再作为 SSH 连接地址；用户在 `明细` 或 `IP分布` 中切换记录不得改变隧道路由。没有合法设备 ID、设备未授权、隧道未登记/不在线、VPS 返回身份或端口不匹配、Windows 系统 SSH 缺失时必须明确失败，不得静默选择其它设备、其它端口或直连访问记录 IP。

正式链路：

```text
board device authorization + device private key
  -> HTTPS register through license.cjwsjzyy.xyz
  -> board system ssh -R 127.0.0.1:<assigned-port>:127.0.0.1:22
  -> dedicated unprivileged VPS tunnel user on the raw VPS SSH endpoint

Factory Client 设备行 -> IP访问记录弹窗 -> 6 位设备 ID
  -> HTTPS query tunnel status through license.cjwsjzyy.xyz
  -> Windows system ssh -J vps3 -p <assigned-port> root@127.0.0.1
  -> VPS loopback reverse tunnel -> board Dropbear host key -> interactive maintenance shell
```

身份与地址规则：

- `IP访问记录` 是服务器观察到的出站请求来源，可能属于运营商 NAT；它不构成设备身份、入站地址或在线证明。隧道唯一定位键是授权链确认的 6 位设备 ID，访问记录选择不得参与端口分配或连接命令。
- 板端使用已有设备私钥和带 `remote_ssh_tunnel` 权限的授权文件签名登记；VPS 必须核对 DNA、设备 ID、公钥指纹和授权有效性，为每台设备分配唯一端口，并把授权 key 约束到 `permitlisten="127.0.0.1:<assigned-port>"`。
- VPS 隧道端口只允许绑定 `127.0.0.1`；专用 `8ax-tunnel` 用户禁止密码、TTY、agent/X11 转发和 shell，只允许 remote forwarding。不得监听 `0.0.0.0:<assigned-port>`，不得占用或修改现有 80/443、授权网关、OTA、xray 或 Cloudflare HTTPS 链路。
- `license.cjwsjzyy.xyz` 继续作为 Cloudflare 代理的 HTTPS 注册/状态入口；SSH 传输必须直连 canonical 配置中的 VPS 原始 SSH endpoint，使用固定公网 IP 或 Cloudflare `DNS only` 记录，不得把 SSH 指向橙云代理地址。板端必须固定校验 VPS SSH host key。
- Factory Client 只启动 Windows 系统 `ssh.exe`，先通过现有 `vps3` 运维别名进入 VPS，再连接该设备的 loopback 分配端口；固定板端账号 `root`、`StrictHostKeyChecking=ask`，并按 6 位设备 ID 使用独立 `HostKeyAlias`。客户端每次连接前必须通过系统 `ssh -G re-board` 解析一个实际存在的 `IdentityFile`，只把该路径传给本次 SSH 进程；旧 Dropbear 所需的 `HostKeyAlgorithms=+ssh-rsa` 和 `PubkeyAcceptedAlgorithms=+ssh-rsa` 也只能作为本次设备连接参数，不得修改 Windows 全局 SSH 配置。客户端不得保存设备密码、复制/保存设备私钥或自动接受板端 host key；`re-board` 未提供可用私钥时必须在启动终端前明确失败。
- 当前设备 ID、VPS 返回的分配端口、隧道在线状态和最终 SSH host key 必须闭合；首次连接由人工核对 host key，已记录 host key 变化时必须由系统 SSH fail-closed。
...
- Factory Client 远程 SSH 登录板端 `root` 使用的本机私钥固定保管为 `D:\授权私钥\remote-ssh-board-root.pem`，Windows SSH 别名 `re-board` 的 `IdentityFile` 必须指向该文件；它与工厂设备授权私钥用途不同，不得上传 VPS、复制到板端、打入客户端或提交 Git。
...
- 驱动内部 H01/H05/H08 等 SDO 整定参数不进入 `drive_parameter_table.tsv`、普通 profile commissioning 或任意对象写入口；V5 不在启动、模型切换、`设置驱动` 或 EtherCAT XML 初始化中写入这些参数。
- 显式逐轴速度前馈维护是唯一受限整定例外。当前型号 profile 只允许登记 `drive.read_velocity_feedforward_source`（`0x2005:0x14`）、`drive.read_velocity_feedforward_filter`（`0x2008:0x13`）、`drive.read_velocity_feedforward_gain` / `drive.write_velocity_feedforward_gain`（`0x2008:0x14`）、`drive.read_communication_eeprom_policy` / `drive.write_communication_eeprom_policy`（`0x200E:0x02`）和已有 `drive.software_reset`；不得接受请求携带任意 index/subindex。动作每次只处理一个已绑定从站，要求 Machine Off/无运动、`H05.19=1`、`H08.18`只读、`H08.19`范围`0.0%..100.0%`且单次最多变化`10.0%`。持久化固定使用 `H0E.01=1 -> 写/读H08.19 -> H0E.01=0 -> software reset -> 全从站OP恢复 -> 再读H08.19/H0E.01/error_code`；任一步失败以同一序列恢复原值，成功或失败都必须保持 Machine Off。
- 操作员明确点击 `复位驱动` 是独立恢复写入例外：只能使用当前型号 profile 已有证据的 `drive.restore_factory_defaults` 对象，在 Machine Off/无运动窗口恢复厂家默认；成功后保持 Machine Off、旧电子齿轮/设零/前馈维护证据失效，并要求重新设置必要的电子齿轮/CSP 模式。该对象不能复用为普通参数写入口。
...
| 厂家远程 SSH | Windows 按钮与启动器在 `8ax-factory-client-source/8ax.FactoryClient/DeviceIpAccessDialog.cs`，详细实现与排障入口记录在 `8ax-factory-client-source/README.md` 的“远程访问板端 SSH”；设备登记/状态 API 与端口/key owner 在 `8ax-factory-client-source/vps-auth-gateway/remote_ssh_gateway.py`；板端 agent 在 `board/services/remote_ssh/`，由 canonical runtime manifest 部署 |
