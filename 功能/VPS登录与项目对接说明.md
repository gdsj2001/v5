...
read_when: [VPS 登录, 授权, private/public 下载, drive profile, 驱动厂家默认整定参数, OTA package, remote relay, WinRemote更新, 厂家远程 SSH]
truth: [授权身份 -> package/profile 或当前设备反向 SSH 登记 -> 板端校验/加载或 VPS tunnel/host-key readback -> consumer, remote relay使用设备独立TLS证书与客户端challenge-response/scoped RAM session, WinRemote更新先验ECDSA P-256分离签名再解析manifest且安装子进程二次验签验hash, WinRemote正式EXE一旦更新必须在同一次发布上传VPS并完成主备域签名三件套回读, 驱动内部位置环/速度环/前馈/滤波/增益等厂家整定SDO不进入参数映射且所有V5链路只读诊断不写, 显式复位驱动可按型号 profile 恢复厂家默认, PDO 固定布局不进入用户参数表]
forbidden: [public fallback 冒充授权, 客户分叉产品代码, 未验签manifest或只验hash的下载, 复用设备授权/SSH/relay私钥作更新签名, 正式publish目录用-SkipUpload生成新EXE或本地更新后不上传VPS, relay明文/匿名/URL token/证书忽略fallback, PowerShell ExecutionPolicy Bypass更新器, 下载验证后安装前不复核的TOCTOU, 把访问记录 IP 当 SSH 地址, Cloudflare 代理 SSH, 自动接受 host key, SSH/FIFO 产品控制旁路, 启动/设置/维护/Profile/XML写 H01/H05/H08 等厂家整定 SDO, 任意index/subindex写入或统一整定值覆盖多轴/多型号, 以跟随误差为由自动调前馈/滤波/增益, 用驱动参数表生成或改写 HAL/XML/PDO, 把复位驱动扩成普通 SDO 写入口]
readback: [license/entitlement, package/profile hash/version, 板端加载 identity, 厂家整定参数只读诊断且不进入readiness期望值, 当前SV630N五轴H08.19恢复厂家原始0.0%且error_code为0/clean restart fresh ready, H0E.01等厂家策略参数只读实际值且无V5期望值, relay certificate fingerprint/device_id/client_id/scopes/session, WinRemote manifest key_id/signature/release_sequence/package size/hash/installer reverify, device tunnel status, SSH host-key/device readback]
impact: [WinRemote与更新发布脚本, remote relay, dealer/factory client, IP访问记录弹窗, drive mapping, 厂家整定参数只读边界, OTA, settings actiond]
acceptance: [身份与产物 hash 闭合；relay明文/匿名/错误MAC/重放/过期/越权/错误证书均fail-closed且正确WinRemote证书固定与scope可走原始路径；WinRemote在解析manifest字段前验证分离签名，包下载后与安装前都验证同一签名manifest绑定的size/hash，篡改manifest/signature/package或回退release_sequence均拒绝；每次正式EXE更新同次上传VPS且VPS原始文件与主备HTTPS的zip/manifest/signature完全一致并验签通过；远程 SSH 必须证明当前设备弹窗 -> 同一设备 ID 的 VPS loopback 隧道 -> 板端 SSH 主机身份与命令回读；启动、设置、维护、Profile和XML均无厂家整定SDO写路径，只读诊断不参与readiness期望值；当前SV630N五轴实验性H08.19恢复厂家原始0.0%、error_code=0并在clean restart后fresh ready，H0E.01等厂家策略参数不再有V5写入口或期望值，专用写链和旧维护证据已删除；复位驱动仅在 Machine Off/无运动窗口执行厂家恢复默认并保持 Machine Off]
...
- 原子写入。
- 返回 SHA256 和字节数读回证据。
- 文件扩展名只允许 `.ngc`、`.nc`、`.tap`、`.gcode`，单文件大小上限与板端 program runtime、relay 和 WinRemote 客户端统一为 `64 MiB`（`67,108,864 bytes`）；空文件拒绝。`Content-Length`、实际接收字节数、写后文件大小和 SHA256 必须一致，任一层不得保留更小或更大的第二上限。
...
- 驱动内部位置环、速度环、前馈、滤波和增益等厂家整定参数一律使用厂家/NVM默认。public/private profile、启动、模型切换、`设置驱动`、维护动作和EtherCAT XML均不得登记或执行这些对象的写命令；只读诊断不得参与motion readiness期望值、自动修复或生成统一多轴参数。当前SV630N实验性`H08.19`仅允许通过已验证旧事务逐轴恢复本机厂家原始`0.0%`，恢复完成并clean restart readback后，专用动作、recovery、Profile命令、部署项、host smoke和持久化证据必须物理删除。
