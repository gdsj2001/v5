...
5. 最近的 focused policy/test 必须在旧入口重新出现在临时 fixture 或受检文本时返回非零，并在当前 canonical checkout 返回 0；禁止通过把旧 token 从审计词表删除、缩小扫描根或把正向命中改称注释来获得通过。
...
~~~text
retired_token	positive_roots	positive_hit_count	allowed_negative_file_and_semantics	canonical_replacement	anti_resurrection_gate	failure_probe_result
~~~
...
allowed_negative_cleanup_locations_semantics_and_removal_condition=
anti_resurrection_gate_and_failure_probe=
...
- doc_owner_files_for_monitor：若 ABI/语义变化，由监控 AI 修改唯一 owner 文档及快读卡并完成文档门禁。
- code_allowed_files：
  - board/services/state_publisher/v5_position_status_publisher.py
  - board/services/state_publisher/v5_machine_status_projection.py
...
- 交接：轻量 AI 先报告拟变 ABI/语义和源码证据；监控 AI 更新文档并通知 hash/磁盘版本后，轻量 AI 重读再改代码。
...
retired_positive_survivor_count_by_source_build_manifest_install_service_test_doc:
canonical_replacement_owner_and_unique_entry_assertions:
anti_resurrection_gate_command_exit_code_and_failure_probe:
