...
V5_SCREENSHOT_TOOL_RULE=板端 UI 截图优先使用 v5 自有的远程 relay 抓帧工具；当前入口为从只读 `D:/v3/tools/v3_remote_frame_capture.py` 搬入并归属 v5 的 `D:/v5/tools/v5_remote_frame_capture.py`。该工具作为 v5 工具维护，后续运行不得引用 `D:/v3`。抓屏图片必须放到 Windows `D:/v5/截图/` 方便人工查看；VM `artifacts/board_ui/` 只作为中间/ignored 输出。直接 `/dev/fb0` 抓帧只能作为 relay 不可用时的诊断证据，不能单独证明产品 UI/operator path 已闭合。
V5_FRAMEBUFFER_CAPTURE_FORMAT_RULE=当前板端实际屏幕和 `/dev/fb0` 诊断抓帧分辨率为 `1024x600`，`bits_per_pixel=24`，`stride=3072`，物理显示按 `rgb24` 解释。`tools/deploy/capture_v5_board_ui.sh` 这类直接 framebuffer 抓屏工具必须把原始行按 `rgb24` 直出，不得按 `bgr24` 交换红蓝通道；每次截图证据必须记录 width/height/bpp/stride/format。若截图颜色与肉眼屏幕不一致，先修抓屏格式转换，不得把错误截图当 UI 证据。
...
PROJECT_PATH_RULE=All project-local paths must be relative to PROJECT_ROOT so the whole v3 folder can be moved intact
SOURCE_FILE_MAX_LINES_RULE=Every human-maintained business/control-flow source owner under PROJECT_ROOT must stay at or below 500 lines. Exempt files include tests, generated files, docs, pure config, pure data tables, constants/enums/maps, style/assets, schema/ABI declarations, and other atomic declarative files with no business/control-flow logic. If an exempt file gains branching business logic, owner decisions, fallback behavior, runtime state transitions, parser/algorithm logic, or command/control flow, it loses the exemption and must be split before exceeding 500 lines.
...
HIGHEST_PARAMETER_TABLE_MEMORY_RULE=Use REQ-PARAM-MEMORY-LIGHTWEIGHT-SAVE in 功能/需求真源索引.md and its owner 功能/0开机参数入内存.md. AGENTS.md keeps precedence and workflow gates; the owner keeps the volatile parameter-table save rule text.
...
- NAS/off-machine backup rsync jobs that only copy existing deliverables or backup folders are not VM sync/build/deploy/operator resources. Do not wait for them, do not stop them, and do not treat them as blockers for VM sync, ARM build, board deploy, or board verification unless they are actively modifying the current PROJECT_ROOT staging/build/deploy directory.
- Every VM/ARM build must use a task-specific staging/build directory and must not reuse unknown old artifacts.
- Board deployment must use the full current PROJECT_ROOT mainline worktree and a freshly identified VM/ARM artifact from the current task-specific staging directory; partial deploys or stale artifacts cannot close board delivery.
...
SOURCE_TRUTH:
- Canonical source is PROJECT_ROOT only. VM_SYNC trees, board `/opt/8ax/...`, staging bundles, deploy outputs, `bak/`, `repo_ignored/`, `artifacts/`, `deploy_tmp/`, old builds, and temp clean copies are not source truth.
- Source fixes must be made in PROJECT_ROOT. If a synced or board copy is temporarily inspected or patched for diagnosis, reproduce the change in PROJECT_ROOT, rebuild from the full current worktree, and redeploy before any source/fix claim.
- Every VM/board sync/build/package/deploy must use the full latest PROJECT_ROOT worktree after `git status`/`git diff` accounts for all modified and relevant untracked source files. Partial copies, partial builds, side branches, stale VM/board copies, stale artifacts, or single-file deploys are diagnostic only and cannot close delivery.
...
- Every human-maintained business/control-flow source owner must stay at or below 500 lines. If such a file is already over 500 lines, any touched slice must reduce it toward compliance by splitting/downshifting ownership or deleting obsolete logic before new behavior is added.
- Business/control-flow files over 500 lines must not accept a new feature owner, new business helper, state machine, parser, algorithm, parameter-migration write path, UI command category, or cross-layer decision.
- Exempt files include tests, generated files, docs, pure config, pure data tables, constants/enums/maps, style/assets, schema/ABI declarations, and other atomic declarative files with no business/control-flow logic.
- An exempt file loses the exemption if it gains branching business logic, owner decisions, fallback behavior, runtime state transitions, parser/algorithm logic, parameter-migration write paths, or command/control flow; split that logic into an owner before the file exceeds 500 lines.
...
- Do not install, enable, configure, or depend on WSL. Linux build/sync/test work must use the configured VM path (`VM_SYNC`) or existing board/VM tools only
...
- Reusable evidence tools belong under `tools/` with focused tests or self-checks. Task-only probes belong under `repo_ignored/<short_task>/scratch/`; temporary board probes belong only under `/tmp/v3_test_tools` or `/run/v3_test_tools` and must never become product runtime paths or hidden fallbacks.
...
VM_PATH_USER=sj
VM_SYNC=/home/sj/Desktop/deliverables/re_plnx_rebuild/work/8ax-native-linuxcnc-current/v3
ARCHIVE_ROOT=repo_ignored

VM_ACCESS:
- Preferred target is the existing VM SSH alias `z20-vm`; do not invent credentials or require `sj@z20-vm` unless that is already configured
- First probe before any sync/build: `ssh z20-vm "whoami; test -d /home/sj/Desktop/deliverables/re_plnx_rebuild/work/8ax-native-linuxcnc-current/v3 && readlink -f /home/sj/Desktop/deliverables/re_plnx_rebuild/work/8ax-native-linuxcnc-current/v3"`
...
- Treat PROJECT_ROOT as canonical source and `VM_SYNC` as the VM build/sync target; after copying to VM, verify path and hashes before using VM build output
